import { AvatarImage, AvatarFallback, Avatar } from "~/components/ui/avatar";
import Link from "next/link";
import { Button } from "~/components/ui/button";
import { CardTitle, CardHeader, CardContent, Card } from "~/components/ui/card";
import Gravatar from "~/components/ui/gravatar";

const options = {
  logo: {
    img_src: "/img/travel-logo.webp",
    website_name: "Andrew G Bliss",
  },
  website_name: "Andrew G Bliss",
  background_cover_url: "/img/jumbotron.jpg",
  tagline: "Let's build something together.",
  headline: "Andrew G Bliss",
  description:
    "Andrew G Bliss is a full-stack developer with a passion for building beautiful websites and applications.",
  website_url: "andrewgbliss.com",
  email: "andrewgbliss@gmail.com",
};

export default function Page() {
  return (
    <main className="flex min-h-screen items-center justify-center bg-gray-100">
      <Card className="w-full max-w-2xl space-y-10">
        <CardContent>
          <div className="grid gap-5">
            <div className="flex items-center space-x-4">
              <Avatar className="h-32 w-32 rounded-full border-2 border-gray-300">
                <Gravatar email={options.email} size={256} />
                <AvatarFallback>AB</AvatarFallback>
              </Avatar>
              <div className="space-y-2 p-5">
                <h2 className="text-3xl font-bold">{options.website_name}</h2>
                <div className="flex space-x-4">
                  <Link href="#" rel="noopener noreferrer" target="_blank">
                    <YoutubeIcon className="h-6 w-6 text-red-600" />
                  </Link>
                  <Link href="#" rel="noopener noreferrer" target="_blank">
                    <TwitterIcon className="h-6 w-6 text-blue-400" />
                  </Link>
                  <Link href="#" rel="noopener noreferrer" target="_blank">
                    <InstagramIcon className="h-6 w-6 text-pink-600" />
                  </Link>
                  <Link href="#" rel="noopener noreferrer" target="_blank">
                    <MailIcon className="h-6 w-6 text-gray-500" />
                  </Link>
                </div>
                <p className="text-gray-500">{options.description}</p>
              </div>
            </div>
            <div className="flex space-x-2">
              <Button className="rounded-full bg-blue-500 px-3 py-1 text-xs text-white">
                HTML
              </Button>
              <Button className="rounded-full bg-blue-500 px-3 py-1 text-xs text-white">
                CSS
              </Button>
              <Button className="rounded-full bg-blue-500 px-3 py-1 text-xs text-white">
                JavaScript
              </Button>
            </div>

            <Card className="w-full">
              <CardHeader>
                <CardTitle>Recent Activity</CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                <p>
                  <strong>1 hour ago:</strong> Lorem ipsum dolor sit amet,
                  consectetur adipiscing elit.
                </p>
                <p>
                  <strong>2 days ago:</strong> Nullam sollicitudin risus sit
                  amet massa scelerisque, et rutrum nulla aliquam.
                </p>
                <p>
                  <strong>1 week ago:</strong> Morbi facilisis, sem quis auctor
                  pretium, elit velit porttitor est, sit amet consequat libero
                  diam id urna.
                </p>
              </CardContent>
            </Card>
          </div>
        </CardContent>
      </Card>
    </main>
  );
}

function InstagramIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <rect width="20" height="20" x="2" y="2" rx="5" ry="5" />
      <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
      <line x1="17.5" x2="17.51" y1="6.5" y2="6.5" />
    </svg>
  );
}

function MailIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <rect width="20" height="16" x="2" y="4" rx="2" />
      <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" />
    </svg>
  );
}

function PhoneIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" />
    </svg>
  );
}

function TwitterIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z" />
    </svg>
  );
}

function YoutubeIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <path d="M2.5 17a24.12 24.12 0 0 1 0-10 2 2 0 0 1 1.4-1.4 49.56 49.56 0 0 1 16.2 0A2 2 0 0 1 21.5 7a24.12 24.12 0 0 1 0 10 2 2 0 0 1-1.4 1.4 49.55 49.55 0 0 1-16.2 0A2 2 0 0 1 2.5 17" />
      <path d="m10 15 5-3-5-3z" />
    </svg>
  );
}
